...
Generate a typed client from the same spec the reference is built from — see [Generating a Client](/openapi-clients) for TypeScript and Python. To be told when something changes instead of polling for it, attach an alert to an insight and have it delivered to [Telegram](/telegram-alerts), Discord or Slack.
