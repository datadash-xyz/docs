...
The identifier columns are what make the log _enriched_: `marketId`, `eventId`, `tagIds` and `seriesId` come resolved on every row, so you can group or filter by market, event or category without a second lookup.
