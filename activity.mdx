...
| **`positionId`**            | The [position](/position-ids) traded.                                        |
...
The identifier columns are what make the log _enriched_. Each one is returned **both as an id and as the resolved entity**, so a row already carries the market's question, the event's title and the category labels:
...
<Note>
  ...
  large fill counts more than a small one. `marketId`, `eventId`, `tagIds` and
  `seriesId` carry through as representative values for the group.
</Note>
