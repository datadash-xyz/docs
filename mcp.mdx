---
title: "MCP Server"
description: "Query Datadash analytics and manage cohorts from Claude, Cursor, VS Code or any MCP client."
---
...
It uses the Streamable HTTP transport and authenticates with the same `X-Api-Key` header as the REST API. If you don't have a key yet, [create one](/api-keys) first.
...
<Tabs>
  <Tab title="Claude Code">
    ```bash
    claude mcp add --transport http datadash https://api.datadash.xyz/mcp --header "X-Api-Key: <your-api-key-here>"
    ```

    This adds the server to the current project only. Add `--scope user` to make it available in every project.

    Run `claude mcp list` to check it connected, or `/mcp` from inside a session.
  </Tab>
  ...
  <Tab title="VS Code">
    Add to `.vscode/mcp.json` in your workspace. The `inputs` entry has VS Code prompt for the key once and store it, so it never sits in the file:
...
<Warning>
  The assistant acts with your key, and a key carries the full access of the account that created it. That includes creating, changing and deleting your cohorts. Give the MCP server a [key of its own](/api-keys#creating-a-key) so you can revoke it without touching anything else.
</Warning>
